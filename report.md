...
|--------|---------|-----------|
| `config.py` | 13 | Σταθερες: host, port, timeouts, RAND παραμετροι |
| `protocol.py` | 59 | Message framing: `send_message()` / `recv_message()` |
| `auction_server.py` | 512 | Auction Server: socket server, user management, auction lifecycle, notifications, checkActive |
| `peer.py` | 484 | Peer/Bidder: register/login/logout, item generation, auction polling, bidding, P2P transaction |
| `run_demo.py` | 54 | Launcher: εκκινει server + 5 peers σε ξεχωριστα processes |
| `test_scenarios.py` | 310 | Ελεγχομενα test σεναρια για ολες τις απαιτουμενες περιπτωσεις |
...
Οι τρεχουσες τιμες ειναι συμμορφες με τις προδιαγραφες:
- `AUCTION_POLL_INTERVAL = 60` (polling καθε 1 λεπτο, Ενοτητα 4.5)
- `ITEM_GEN_MAX_INTERVAL = 120` (RAND*120 sec, Ενοτητα 4.2)
- `BID_INTEREST_PROBABILITY = 0.60` (60% πιθανοτητα ενδιαφεροντος, Ενοτητα 4.6)
- `BID_INCREMENT_FACTOR = 0.10` (NewBid = Highest * (1 + RAND/10), Ενοτητα 4.7)
- `CHECK_ACTIVE_INTERVAL = 5` (ελεγχος ζωντανιας πωλητη καθε 5s)

Για γρηγοροτερο demo μπορειτε να μειωσετε τα AUCTION_POLL_INTERVAL και
ITEM_GEN_MAX_INTERVAL (π.χ. σε 10 και 30 αντιστοιχα).

### Αποκλισεις απο τις Προδιαγραφες

Δεν υπαρχουν ουσιαστικες αποκλισεις. Ολες οι λειτουργιες υλοποιηθηκαν
οπως περιγραφονται στην εκφωνηση.
...
[alice] <- success=True  token=264246  msg='Already logged in, returning existing token.'
```
...
συμπεριφορα:
- Αυτοματη παραγωγη αντικειμενων (RAND * 120s intervals)
- Αυτοματο polling (καθε 60s)
