...
def start_listener(name):
    ctx = {"running": True, "tcp_port": 0, "udp_port": 0, "udp_sock": None}
    _listeners[name] = ctx
    usock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    usock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    usock.bind(("127.0.0.1", 0))
    ctx["udp_sock"] = usock
    ctx["udp_port"] = usock.getsockname()[1]

    srv = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    srv.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    srv.bind(("127.0.0.1", 0))
    ctx["tcp_port"] = srv.getsockname()[1]
    srv.listen(5)
    srv.settimeout(1.0)
    ...
    def handle(sock):
        try:
            ...
            if t == "CHECK_ACTIVE":
                ...
                pr("[%s] AUCTION.WON       | item='%s'  final_price=%.2f  seller=%s:%s udp=%s" % (
                    name, msg["object_id"], msg["final_bid"],
                    msg["seller_ip"], msg["seller_port"],
                    msg.get("seller_udp_port", msg["seller_port"])))
                send_message(sock, {"type": "ACK"})
                threading.Thread(target=_do_buy, args=(
                    name, msg["object_id"], msg["final_bid"],
                    msg["seller_ip"], msg["seller_port"],
                    int(msg.get("seller_udp_port", msg["seller_port"]))), daemon=True).start()
            elif t == "AUCTION_SOLD":
    ...
    time.sleep(0.3)
    return ctx["tcp_port"], ctx["udp_port"]
...
def _do_buy(buyer, oid, bid, sip, sport, seller_udp_port):
    time.sleep(0.5)
    try:
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.settimeout(5)
        s.connect((sip, int(sport)))
        bu = udp_ports.get(buyer, 0)
        ...
        if not r.get("success"):
            ...
            return
        if r.get("udp"):
            ctx = _listeners.get(buyer, {})
            usock = ctx.get("udp_sock")
            if not usock:
                pr("[%s] TRANSACTION.FAIL  | no UDP socket" % buyer)
                return
            meta_bytes = gbn_udp.receive_metadata(
                usock,
                drop_data_prob=0.0,
                ack_send_prob=1.0,
                log=lambda m: pr("[%s] %s" % (buyer, m)),
                deadline=time.time() + 60.0,
            )
            if meta_bytes is None:
                pr("[%s] TRANSACTION.FAIL  | UDP incomplete" % buyer)
                return
        ...
        if buyer in tokens:
            try:
                send_req({"type": "TRANSACTION_REPORT",
                          "token_id": tokens[buyer], "object_id": oid,
                          "outcome": "success"})
                pr("[%s] TRANSACTION_REPORT success for '%s'" % (buyer, oid))
            except Exception:
                pass
...
def _do_sell(seller, sock, msg):
    ...
    send_message(sock, {"type": "TRANSACTION_RESP", "success": True,
                         "object_id": oid, "metadata": "", "udp": True})
    try:
        sock.close()
    except OSError:
        pass
    ok = gbn_udp.GbnUdpSender(
        usock, (buyer_ip, buyer_udp), packets,
        log=lambda m: pr("[%s] %s" % (seller, m)),
    ).run()
    if ok:
        os.remove(fp)
...
def main():
    ...
    port_a, udp_a = start_listener("alice")
    udp_ports["alice"] = udp_a
    ...
    ports["eve"], udp_ports["eve"] = start_listener("eve")
